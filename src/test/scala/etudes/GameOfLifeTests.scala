...
class GameOfLifeTests extends AnyFlatSpec with should.Matchers{
  ...
  "οοοοο οοοοο οοοοο" should "" in {
    ...
    println()
    gameOfLife.gen(dessert).take(10).zipWithIndex.foreach((g,index) => {
      println(s"generation ${index+2}")
      println()
      g.printRows(default = gameOfLife.emptySymbol, ignoreDefaultLines = true)
      println(" " * 100)
    })
